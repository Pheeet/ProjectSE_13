    ...
    <div class="layout">
      ...
      <aside class="panel sidebar">
        <div class="filter-group">
          ...
          <div class="toolbar">
            <select v-model="state.yearStart" style="flex: 1">
              <option value="">ตั้งแต่ปี</option>
              <option v-for="y in years" :key="'start-' + y" :value="y">
                {{ y }}
              </option>
            </select>
            <select v-model="state.yearEnd" style="flex: 1">
              <option value="">ถึงปี</option>
              <option v-for="y in years" :key="'end-' + y" :value="y">
                {{ y }}
              </option>
            </select>
          </div>
        ...
        <div class="results">
          ...
          <div class="card">
            ...
            <div class="result-list">
              <div
                v-if="!results.length"
                class="result-item small"
                style="text-align: center; padding: 100px 10px"
              >
              ...
              >
                ...
                <div class="small">
                  {{ r.year }} • {{ r.type }} • {{ r.degree || "—" }} •
                  {{ r.category }} • {{ r.advisor }}
                </div>

                <!-- INLINE ACTIONS (แสดงเมื่อรายการนี้ถูกเลือก) -->
                <transition name="fade">
                  <div
                    v-if="selected && selected.id === r.id"
                    class="inline-actions"
                  >
                    <button
                      class="primary btn"
                      :disabled="!r.pdfUrl"
                      @click.stop="openLink(r.pdfUrl)"
                    >
                      ดู PDF
                    </button>
                    <button
                      class="btn"
                      :disabled="!r.posterUrl"
                      @click.stop="openLink(r.posterUrl)"
                    >
                      ดู Poster
                    </button>
                    <span class="small muted" style="margin-left: auto"
                      >* ปุ่มเทาถ้าไฟล์ยังไม่พร้อม</span
                    >
                  </div>
                </transition>
              </div>
          ...
          <div class="card preview">
            ...
            <div
              v-if="!selected"
              class="skeleton"
              style="height: 480px; margin-top: 8px"
            ></div>
            ...
            <div v-else style="margin-top: 8px">
              ...
              <div class="small">
                ...
                {{ selected.degree || "—" }} • {{ selected.year }}
              </div>
              <div style="margin-top: 8px">
                ที่ปรึกษา: {{ selected.advisor }}
              </div>
              ...
              <div class="chips" style="margin: 10px 0">
                ...
                <span class="badge" v-if="selected.degree">{{
                  selected.degree
                }}</span>
              </div>
              ...
              <div class="toolbar" style="gap: 10px">
                <button
                  ...
                  @click="openLink(selected.pdfUrl)"
                >
                  เปิด PDF
                </button>
                <button
                  class="btn"
                  :disabled="!selected.posterUrl"
                  @click="openLink(selected.posterUrl)"
                >
                  เปิด Poster
                </button>
              </div>
        ...
        <div class="panel" style="margin-top: 14px">
          ...
          <div class="related-table-container">
            <table class="table">
              <thead>
                <tr>
                  <th style="width: 34%">ชื่อเรื่อง</th>
                  <th>ปี</th>
                  <th>Category</th>
                  <th>Type</th>
                  <th>Degree</th>
              ...
              <tbody>
                <tr v-if="!selected">
                  <td
                    colspan="6"
                    class="small"
                    style="color: #8b9099; text-align: center"
                  >
                    — เลือกงานวิจัยจากด้านบนเพื่อดูงานวิจัยที่เกี่ยวข้อง
                  </td>
                </tr>
                <tr v-else-if="!relatedResults.length">
                  <td
                    colspan="6"
                    class="small"
                    style="color: #8b9099; text-align: center"
                  >
                    — ไม่พบงานวิจัยที่เกี่ยวข้องตามตัวกรองทั้งหมด
                  </td>
                </tr>
